...
namespace FSAutomator.BackEnd
{
    ...
    {
        internal bool ExportAutomation(string fileNameSetByUser, string destinationPath, ObservableCollection<FSAutomatorAction> l_ActionList, AutomationFile l_SAutomationFilesList)
        {
            if (l_ActionList is not null && l_ActionList.Count() > 0)
            {
                if (l_ActionList[0].Name == "DLLAutomation")
                {
                    ...
                    var SavedDLLFileName = Path.Combine(destinationPath, DLLFileName) + ".dll";
                    ExportDLL(DLLFileName, SavedDLLFileName);
                }
                else   //this is a JSON, maybe within a pack, maybe standalone
                {
                    ...
                    if (dllFilesInAction.Any()) //is a pack
                    {
                        ...
                        if (allDLLsExist)
                        {
                            ...
                            ExportPack(fileNameSetByUser, packageName, automationFileName, l_SAutomationFilesList, json);

                        }
                    }
                    else //standalone json
                    {
                        var jsonFileName = Path.Combine(destinationPath, Path.GetFileNameWithoutExtension(fileNameSetByUser) + ".json");
        ...
        private void ExportPack(string fileNameSetByUser, string packageName, string automationFileName, AutomationFile l_SAutomationFilesList, string json)
        {
            var tempDirPath = Path.Combine(@"Temp", fileNameSetByUser);
            var JSONPath = Path.Combine(tempDirPath, l_SAutomationFilesList.FileName);
            var newJSONPath = Path.Combine(tempDirPath, fileNameSetByUser + ".json");
            ...
            var zipPath = Path.Combine(Directory.GetParent(fileNameSetByUser).ToString(), @"Exports", fileNameSetByUser + ".zip");
